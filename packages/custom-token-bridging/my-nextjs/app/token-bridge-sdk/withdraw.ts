...
import { getArbitrumNetwork, registerCustomArbitrumNetwork, ParentToChildMessageStatus, Erc20Bridger, ChildTransactionReceipt, ChildToParentMessageStatus} from '@arbitrum/sdk';
import {percentIncrease} from "@/token-bridge-sdk/utils";
import { customConduitNetwork } from '@/config/network';
import {parentTokenAddr, childProvider, parentProvider, childTokenAddr, PROXY_KEY, OPERATOR_KEY, balanceOfERC20} from '@/token-bridge-sdk/utils';
...
export async function withdraw(
    ...
    try {
      ...
      const erc20Bridger = await Erc20Bridger.fromProvider(parentProvider);
      /**
      ...
      const request = await erc20Bridger.getWithdrawalRequest({
        ...
        erc20ParentAddress: childTokenAddr,
        destinationAddress: await parentWallet.getAddress(),
        amount: tokenWithdrawAmount
      })
      console.log(await childOperator.getAddress(), childTokenAddr, await parentWallet.getAddress(), ethers.utils.formatUnits(tokenWithdrawAmount, 18), request);
